...
function drawHUD(){
  // Top-left: username + coins (always visible during game)
  ctx.fillStyle='rgba(5,8,18,.82)';rr(ctx,8,8,220,36,8);ctx.fill();
  ctx.font='bold 12px Nunito';ctx.textAlign='left';
  ctx.fillStyle='#f7c948';ctx.fillText('👤 '+(window.currentUser?.username||''),18,23);
  const totalCoins=((window.G?.earnedCoins||0)+(window.currentUser?.coins||0));
  ctx.fillStyle='#f7c948';ctx.font="bold 11px 'Bangers',cursive";
  ctx.fillText('🪙 '+totalCoins.toLocaleString(),18,40);
  // Centre stats bar
  ctx.fillStyle='rgba(5,8,18,.82)';rr(ctx,W/2-320,8,640,36,8);ctx.fill();
  ctx.font='bold 13px Nunito';ctx.textAlign='center';
  const w=curWpn();
  ctx.fillStyle='#c0392b';ctx.fillText('❤ '+Math.max(0,Math.round(G.playerHp))+'/'+G.playerMaxHp,W/2-250,31);
  ctx.fillStyle='#7ecff7';ctx.fillText('Welle '+G.wave,W/2-90,31);
  ...
  ctx.fillStyle='#27ae60';ctx.fillText('☠ '+G.kills,W/2+30,31);
  const secs2=Math.floor(G.gameTime/60);ctx.fillStyle='#9db4c8';ctx.fillText('⏱ '+Math.floor(secs2/60)+':'+(secs2%60).toString().padStart(2,'0'),W/2+140,31);
  ...
  if(w){ctx.fillStyle='rgba(5,8,18,.82)';rr(ctx,W-185,8,175,50,8);ctx.fill();ctx.fillStyle='#f7c948';ctx.font='bold 11px Nunito';ctx.textAlign='left';ctx.fillText(w.icon+' '+w.name,W-175,25);if(w.id==='minigun'){if(w.cooling){ctx.fillStyle='#e74c3c';ctx.fillText('COOLDOWN',W-175,42);}else{ctx.fillStyle='#aaa';ctx.fillText('∞',W-175,42);}}else if(w.reloading){ctx.fillStyle='#e67e22';ctx.fillText('LADEN '+(((1-w.reloadTimer/w.reloadTime)*100)|0)+'%',W-175,42);}else{ctx.fillStyle=w.ammo<=w.maxAmmo*.25?'#e74c3c':'#aaa';ctx.fillText(w.ammo==='999'||w.ammo===999?'∞':w.ammo+'/'+w.maxAmmo,W-175,42);}}
  ...
  ctx.fillStyle='rgba(150,170,190,.35)';ctx.font='10px Nunito';ctx.textAlign='center';ctx.fillText('[ESC] Pause    [R] Nachladen    [1-8] Waffe    [Shift] Dash',W/2,H-8);
  // acct-bar DOM is now above canvas – update it too
