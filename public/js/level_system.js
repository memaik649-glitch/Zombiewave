...
async function showLevelComplete() {
  ...
  if (el('lc-next-btn')) {
    // On easy: show button but it will trigger jumpscare. On normal+: show if unlocked
    const showNext = nextLvl && (selectedDiff === 'easy' || isLevelUnlocked(nextLvl.id));
    el('lc-next-btn').style.display = showNext ? 'inline-block' : 'none';
  }
...
async function goNextLevel() {
  ...
  // EASY MODE: trigger jumpscare instead of going to next level
  if (selectedDiff === 'easy') {
    // Hide the level-complete overlay first
    const lcOv = document.getElementById('ov-level-complete');
    if (lcOv) lcOv.classList.add('h');
    // Trigger the jumpscare sequence – after it's done, drop back to menu
    if (typeof triggerJumpscare === 'function') {
      triggerJumpscare(() => {
        // After jumpscare: just go to menu
        gameRunning = false;
